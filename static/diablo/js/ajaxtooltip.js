...
//Updated November 2012 to use jquery 1.6
...
var ajaxtooltip={
  ...
  positiontip:function($tooltip, e){
    ...
    var half = $tooltip.get(0).offsetHeight/2
    tipx=(e.clientX+twidth>docwidth)? tipx-twidth-(2*this.useroffset[0]) : tipx //account for right edge
    tipy=(e.clientY+theight>docheight)? tipy-theight-(2*this.useroffset[0])+half : tipy-half //account for bottom edge
...
jQuery(document).ready(function(){
  ...
  $('*[title^="ajax:"],*[title^="notajax:"]').each(function(index){ //find all links with "title=ajax:" declaration
    this.titleurl=jQuery.trim(this.getAttribute('title').split('ajax:')[1]) //get URL of external file or selector
    if (!!this.getAttribute('title').match("^ajax")) {this.is_ajax = true;}
	...
	if (!$tooltip.get(0).loadsuccess) {
	  ...
	  if (!this.is_ajax) {
	    $tooltip.html($(this.titleurl).html());
